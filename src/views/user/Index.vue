                ...
                >
                  ...
                  <template v-slot:default="row">
                    ...
                    <td class="align-middle text-center">
                      <router-link :to="`/user/edit/${row.item.id}`">
...
export default defineComponent({
  ...
  components: {},
  data() {
    return { users: [], lastPage: 1, total: 0, isOpenModal: false, currentData: { id: '', name: '' }, keyword: null }
  },
  async mounted() {
    await this.search()
  },
  ...
  methods: {
    ...
    async search(keyword = null) {
      ...
      try {
        const res = await UserService.search(this.$axios, this.$store, keyword)
        if (res.success) {
    ...
    async submitDelete() {
      this.$store.dispatch('startLoading')
      try {
        const res = await UserService.deleteOne(this.$axios, this.currentData.id)
        if (res.success) {
          await this.search()
        }
